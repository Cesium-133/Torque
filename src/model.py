...
def create_torque_regressor(input_shape=(10, 7), output_dim=2, model_type='gru', n_frames=10, m_frames=2):
    ...
    if model_type == 'gru':
        ...
        # 第一层GRU，返回序列以便后续层处理
        x = GRU(units=512, return_sequences=True, dropout=0.2, recurrent_dropout=0.2, name='gru_1')(x)
        
        # 第二层GRU，只返回最后的输出
        gru_output = GRU(units=256, return_sequences=False, dropout=0.2, recurrent_dropout=0.2, name='gru_2')(x)
        
        # Position Embedding: 将窗口位置编码映射到与GRU输出相同的维度
        # 假设最大位置数为1000，可以根据实际需要调整
        position_embedding = Embedding(input_dim=1000, output_dim=256, name='position_embedding')(position_input)
    ...
    #print(f"Model output shape: {output_dim} (predicting future {m_frames} frames)")
